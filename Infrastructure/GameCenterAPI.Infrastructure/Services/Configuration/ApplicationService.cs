...
namespace GameCenterAPI.Infrastructure.Services.Configuration
{
    public class ApplicationService : IApplicationService
    {
        public List<Menu> GetAuthorizeDefinitionEndpoints(Type type)
        {
            ...
            var controllers = assembly.GetTypes().Where(t => t.Namespace == "GameCenterAPI.API.Controllers" && t.BaseType.Name.Contains("Controller"));

            List<Menu> menus = new();

            foreach (var controller in controllers)
            {
                var actions = controller.GetMethods().Where(t => t.IsDefined(typeof(AuthorizeDefinitionAttribute), true));
                if (actions != null)
                {
